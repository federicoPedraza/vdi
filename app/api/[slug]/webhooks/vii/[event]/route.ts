...
export async function POST(
  ...
  console.log(`Received webhook for ${slug}: ${event}`, body);
  try {
    const convexUrl = process.env.NEXT_PUBLIC_CONVEX_URL || "https://famous-firefly-743.convex.cloud";
    const convex = new ConvexHttpClient(convexUrl);

    // Resolve partner by slug (public endpoint for incoming webhooks)
    const partner = await convex.query(api.authDb.getPartnerBySlug as any, { slug });
    if (!partner?._id) {
